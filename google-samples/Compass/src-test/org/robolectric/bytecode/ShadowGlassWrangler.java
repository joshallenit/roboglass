...
import org.robolectric.bytecode.MethodSignature;
...
public class ShadowGlassWrangler extends ShadowWrangler {
  ...
  private static final boolean STRIP_SHADOW_STACK_TRACES = false;
  ...
  public ShadowGlassWrangler(ShadowMap shadowMap, Setup setup) {
	  super(shadowMap);
  ...
  public void classInitializing(Class clazz) {
    ...
    Logger.d("classInitializing %s, shadow %s", clazz, shadowClass);
    System.out.println("classInitializing %s, shadow %s" + Arrays.toString(new Object[]{clazz, shadowClass}));
    if (shadowClass != null) {
      try {
        ...
        method.invoke(null);
    	  ...
    	  ClassInfo info = new AsmInstrumentingClassLoader.AsmClassInfo(clazz.getName(), null); 
    	  if (!setup.containsStubs(info)) {
  ...
  synchronized private ShadowConfig getShadowConfig(Class clazz) {
    ...
    if (shadowConfig == null) {
      ...
      System.out.println("shadowConfig "+clazz+" = "+shadowConfig);
      return shadowConfig;
    } else {
    	System.out.println("shadowConfig "+clazz+" = "+shadowConfig);
